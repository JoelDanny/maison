...
<html>
    ...
    <title>reservation achat de maison</title>
    </head>
</html>
    ...
    <div class="container" style="margin-top:2cm; background-color:white;"> 
        ...
        <table class="table table-striped table-bordered" border="3">
            ...
            <tbody>
                ...
                $db= Database::connect();
                $statment = $db->query('SELECT * FROM listelocation');
                while($listelocation = $statment->fetch()){

                   echo' <tr>';
                   echo' <td>'.$listelocation['nom_location'].'</td>';
                   echo' <td>'.$listelocation['prenom_location'].'</td>';
                   echo' <td>'.$listelocation['email_location'].'</td>';
                   echo' <td>'.$listelocation['maison_locatio'].'</td>';
                   echo' <td>'.$listelocation['phone_location'].'</td>';
                   echo' <td>'.$listelocation['today'].'</td>';
